...
describe("SessionService", () => {
  ...
  describe("submitHint", () => {
    it("should successfully submit a valid hint and return hint details", async () => {
      ...
      (mockPrisma.hintUsage.findMany as jest.Mock).mockResolvedValue([{ hint: { progressRate: 20 } }]);
    ...
    it("should throw SessionNotFoundError if session does not exist (BR-06)", async () => {
      ...
      await expect(sessionService.submitHint("invalidSession", "HINT01")).rejects.toThrow(SessionNotFoundError);
    });
    ...
    it("should throw HintNotFoundError if hint does not exist (BR-06)", async () => {
      ...
      await expect(sessionService.submitHint(sessionId, "NONEXISTENT")).rejects.toThrow(HintNotFoundError);
    });
    ...
    it("should throw HintThemeMismatchError if hint exists but is for a different theme (BR-06)", async () => {
      ...
      await expect(sessionService.submitHint(sessionId, code)).rejects.toThrow(HintThemeMismatchError);
    });
    ...
    it("should throw HintInactiveError if hint exists but is inactive (BR-06)", async () => {
      ...
      await expect(sessionService.submitHint(sessionId, code)).rejects.toThrow(HintInactiveError);
    });
  });
});
