...
└── UI 참조 (UI Reference): 각 단계별 화면 요소 및 인터랙션
```
...
- **사용자 행동**:
  - 플레이어 화면 하단의 "EscapeHint" 로고를 5회 연속 클릭
  - (숨겨진 기능, 플레이어에게 노출되지 않음)
- **시스템 반응**:
  - 로고 클릭 시마다 진동 피드백 (모바일) 또는 시각적 피드백 (작은 애니메이션)
  - 5회 클릭 완료 시 비밀번호 입력 다이얼로그 표시
- **화면 요소**:
  - 다이얼로그 제목: "관리자 로그인"
...
- **사용자 행동**: 비밀번호 "admin2024!@" 입력 후 [로그인] 클릭
- **시스템 반응**:
  - 로딩 스피너 (500ms)
  - 서버 인증 (POST /api/admin/auth/login)
  - JWT 토큰 발급 및 HttpOnly 쿠키에 저장
  - 관리자 대시보드로 리다이렉트
- **예상 소요 시간**: 2초
...
│  테마 이름 *                         │
│  ┌─────────────────────────────┐   │
│  │ 해적선의 보물_____________    │   │
│  └─────────────────────────────┘   │
│                                     │
│  테마 설명                           │
│  ┌─────────────────────────────┐   │
│  │ 해적선에서 숨겨진 보물을      │   │
│  │ 찾아라!___________________    │   │
│  └─────────────────────────────┘   │
│                                     │
│  제한 시간 (분) *                    │
│  ┌─────────┐                        │
...
- **시스템 반응**:
  ...
  - 서버 전송 (POST /api/admin/themes/:themeId/hints)
  - 서버 검증 (힌트 코드 중복 체크)
...
- **시스템 반응**:
  ...
  - 서버에 순서 업데이트 (PATCH /api/admin/hints/:id/order)
  - 성공 메시지: "✅ 순서가 변경되었습니다."
- **예상 소요 시간**: 5초
...
1. **UI/UX 디자인**: 와이어프레임 및 목업 작성 (Figma)
2. **API 명세서**: RESTful API 엔드포인트 상세 정의
3. **테스트 시나리오**: 본 문서 기반으로 QA 테스트 케이스 작성
4. **개발 착수**: Phase 1 개발 시작 (2025-11-26)
